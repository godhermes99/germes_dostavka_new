...
=======
import 'dart:async';
import 'dart:ui'; // 🔥 ДОДАНО ДЛЯ ЕФЕКТУ РОЗМИТТЯ
import 'package:flutter/material.dart';
import 'package:provider/provider.dart';
import '../core/supabase_service.dart';
import '../providers/cart_provider.dart';
...
class _RestaurantMenuScreenState extends State<RestaurantMenuScreen>
  ...
  // ========================================================================
  // 🔥 ДІАЛОГОВЕ ВІКНО З ЕФЕКТОМ РОЗМИТТЯ ФОНУ ТА ВИПРАВЛЕНИМИ ДУЖКАМИ
  // ========================================================================
  void _showDishDetails(BuildContext context, Map<String, dynamic> originalDish, bool isOpen) {
    final Map<String, dynamic> dish = Map<String, dynamic>.from(originalDish);
    ...
    showDialog(
      context: context,
      barrierColor: Colors.black54, // Злегка затемнюємо фон
      builder: (context) {
        return BackdropFilter(
          // 🔥 ОСЬ ВОНО! Сильне розмиття заднього фону
          filter: ImageFilter.blur(sigmaX: 8.0, sigmaY: 8.0),
          child: StatefulBuilder(
              builder: (context, setModalState) {
                final isFav = _favoriteDishIds.contains(dish['id']);
